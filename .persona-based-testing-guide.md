...
```text
Environment Check: Check for environment token context FIRST!
Run: env | grep -i simple
If SIMPLE_SECRETS_TOKEN is set, note that it will auto-authenticate commands.
Either unset it for security testing or be aware it's providing authentication.

**Installation Context Check:**
Run: ls ~/.simple-secrets
Result determines ADDITIONAL testing context, but ALL scenarios must be tested:

**TESTING STRATEGY (ALWAYS DO BOTH)**:
1. **Fresh Installation Testing**: Use isolated environment to test setup flow
   ```bash
   export SIMPLE_SECRETS_CONFIG_DIR=/tmp/test-fresh
   rm -rf /tmp/test-fresh
   # Test first-run experience, setup flow, initial user creation
   ```
2. **Existing Installation Testing**: Test established user workflows
   ```bash
   # Use existing ~/.simple-secrets for day-to-day operations testing
...
✅ **Always show these error patterns:**
