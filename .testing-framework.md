...
- [ ] **Config template**: `cat ~/.simple-secrets/config.json` - verify commented examples and rotation_backup_count
...
- [ ] **Default behavior**: `./simple-secrets get test-key` → prints to stdout only
- [ ] **Clipboard only**: `./simple-secrets get test-key --clipboard` → copies to clipboard AND prints to stdout
- [ ] **Silent only**: `./simple-secrets get test-key --silent` → no stdout output
- [ ] **Clipboard and silent**: `./simple-secrets get test-key --clipboard --silent` → clipboard only, no stdout
- [ ] **Short flags**: `./simple-secrets get test-key -c -s` → same as --clipboard --silent
- [ ] **Combined flags**: `./simple-secrets get test-key -cs` → same as -c -s
- [ ] **Clipboard error handling**: Test clipboard functionality in environments without clipboard utilities
- [ ] **Nonexistent with flags**: `./simple-secrets get nonexistent -c -s` → proper error handling with flags
