# Simple-Secrets CLI Testing Framework
...
- [ ] **Command context preservation**: All errors show which command failed
...
### 🔍 **NEW: Rotation Tracking Validation**

- [ ] **Rotation timestamps updated**: Verify `LastRotatedAt` field updated for all secrets
- [ ] **Rotation count incremented**: Check `RotationCount` increased by 1 for existing secrets
- [ ] **Modified timestamps preserved**: Verify `ModifiedAt` NOT changed during rotation (preserves user modification history)
- [ ] **Created timestamps preserved**: Verify `CreatedAt` unchanged for existing secrets
- [ ] **New secret rotation data**: New secrets created during rotation have `RotationCount: 1`
- [ ] **JSON metadata validation**: Check `~/.simple-secrets/secrets.json` contains `last_rotated_at` and `rotation_count` fields
- [ ] **Disabled state preserved**: Disabled secrets remain disabled with correct rotation tracking
- [ ] **Size updated correctly**: Secret size reflects current plaintext length (handles corruption fixes)
...
- [ ] **Unauthenticated completion**: Commands show graceful fallback when not authenticated
...
### 🔐 **Configuration Security & Data Integrity Validation**

> **Critical**: Validate that all configuration files, storage, and security mechanisms work as intended. These tests ensure secrets remain encrypted, configurations behave correctly, and data integrity is maintained.

#### Secret Storage Security

- [ ] **Secrets never stored in plaintext**:
  - Create secret, examine `~/.simple-secrets/secrets.json` directly
  - Verify all secret values are base64-encoded encrypted data (not plaintext)
  - Search for plaintext secret values in ALL config files: `grep -r "actual-secret-value" ~/.simple-secrets/`
  - ❌ CRITICAL FAILURE if any plaintext secrets found

- [ ] **Token hashing verification**:
  - Create user, examine `~/.simple-secrets/users.json` directly
  - Verify tokens are stored as hashes (base64 encoded, not plaintext tokens)
  - Search for plaintext tokens in ALL config files: `grep -r "actual-token-value" ~/.simple-secrets/`
  - ❌ CRITICAL FAILURE if any plaintext tokens found

- [ ] **Master key security**:
  - Verify master key never appears in plaintext in any config file
  - Check that `~/.simple-secrets/master.key` exists with proper permissions (600)
  - Verify master key file contains encrypted/encoded data, not plaintext key material

#### Post-Rotation Validation

- [ ] **Secrets change after master key rotation**:
  - Store test secret: `simple-secrets put test-key "test-value"`
  - Record encrypted value from `secrets.json`: `cp ~/.simple-secrets/secrets.json /tmp/before-rotation.json`
  - Rotate master key: `simple-secrets rotate master-key`
  - Compare encrypted values: encrypted data MUST be different in `secrets.json`
  - Verify secret still decrypts correctly: `simple-secrets get test-key` returns "test-value"
  - ❌ CRITICAL FAILURE if encrypted values are identical after rotation

- [ ] **Token hashes change after token rotation**:
  - Create user: `simple-secrets create-user testuser reader`
  - Record token hash from `users.json`: copy the hash value
  - Rotate token: `simple-secrets rotate token testuser`
  - Verify hash changed in `users.json` (must be different base64 string)
  - Verify old token no longer works, new token works
  - ❌ CRITICAL FAILURE if hash is identical after rotation

- [ ] **Backup integrity after rotation**:
  - Rotate master key (creates backup): `simple-secrets rotate master-key`
  - Verify backup directory created with timestamp format
  - Check backup contains: `secrets.json`, `users.json`, `master.key`, `roles.json`
  - Verify backup secrets are encrypted with OLD key (different from current)
  - Test restore from backup works: `simple-secrets restore database <backup-name>`

#### Environment Variable Configuration

- [ ] **SIMPLE_SECRETS_TOKEN environment variable**:
  - Set: `export SIMPLE_SECRETS_TOKEN=valid-token`
  - Test command works without `--token` flag: `simple-secrets list keys`
  - Unset env var: `unset SIMPLE_SECRETS_TOKEN`
  - Verify command fails without token: `simple-secrets list keys` (should require auth)

- [ ] **SIMPLE_SECRETS_DATA_DIR environment variable**:
  - Set custom dir: `export SIMPLE_SECRETS_DATA_DIR=/tmp/custom-secrets-test`
  - Run setup: `simple-secrets setup`
  - Verify data created in custom directory: `ls /tmp/custom-secrets-test/`
  - Verify original `~/.simple-secrets/` not affected
  - Test operations work with custom directory
  - Cleanup: `rm -rf /tmp/custom-secrets-test && unset SIMPLE_SECRETS_DATA_DIR`

- [ ] **SIMPLE_SECRETS_CONFIG_DIR environment variable**:
  - Set custom config dir: `export SIMPLE_SECRETS_CONFIG_DIR=/tmp/custom-config-test`
  - Run operations, verify config files created in custom location
  - Verify isolation from default config directory
  - Cleanup: `rm -rf /tmp/custom-config-test && unset SIMPLE_SECRETS_CONFIG_DIR`

#### Configuration File Behavior

- [ ] **JSON config file validation**:
  - Create `~/.simple-secrets/config.json` with valid settings
  - Test settings are respected (e.g., `rotation_backup_count`)
  - Create config with invalid JSON syntax - verify graceful error handling
  - Create config with invalid values - verify validation errors
  - Test config file permissions (should be readable by user only)

- [ ] **YAML config file validation** (if supported):
  - Create `~/.config/simple-secrets/config.yaml` with settings
  - Verify YAML config is parsed and respected
  - Test precedence: environment variables override config file settings
  - Test both JSON and YAML present - verify precedence rules

- [ ] **Configuration precedence testing**:
  - Set value in config file: `rotation_backup_count: 2`
  - Override with environment: `export SIMPLE_SECRETS_ROTATION_BACKUP_COUNT=3`
  - Override with command flag (if available)
  - Verify correct precedence: CLI flags > env vars > config file > defaults

#### Backup Validation

- [ ] **Automatic backup creation**:
  - Before master key rotation: note current timestamp
  - Rotate master key: `simple-secrets rotate master-key`
  - Verify backup directory created: `ls ~/.simple-secrets/backups/`
  - Verify backup name format: `rotate-YYYYMMDD-HHMMSS`
  - Verify backup contains ALL required files (complete system state)

- [ ] **Backup completeness**:
  - Create complex state: multiple users, secrets, disabled items
  - Create manual backup: `simple-secrets backup` (if available)
  - Verify backup contains: secrets, users, roles, master key, configuration
  - Verify backup file permissions are secure (600)
  - Test backup in isolation: copy to clean system, verify restore works

- [ ] **Backup retention**:
  - Set `rotation_backup_count: 2` in config
  - Perform 3 master key rotations
  - Verify only 2 most recent backups remain
  - Verify oldest backup was automatically cleaned up

#### File Permissions & Security

- [ ] **Secure file permissions**:
  - Verify `~/.simple-secrets/` directory is 700 (user-only access)
  - Verify all JSON files are 600 (user read/write only)
  - Verify master key file is 600 (user read/write only)
  - Test that other users cannot read secret files: `sudo -u nobody cat ~/.simple-secrets/secrets.json` (should fail)

- [ ] **Configuration directory security**:
  - Verify config directories have proper permissions
  - Test behavior when config directory is not writable
  - Test behavior when config files are not readable
  - Verify graceful error handling for permission issues

#### Data Integrity Validation

- [ ] **Cross-file consistency**:
  - Create users and secrets in mixed order
  - Verify user references in secrets are valid
  - Verify role assignments are consistent across files
  - Test that orphaned references are handled gracefully

- [ ] **Concurrent operation safety**:
  - Run multiple operations simultaneously (if safe):
    - Terminal 1: `simple-secrets put key1 value1`
    - Terminal 2: `simple-secrets put key2 value2`
  - Verify both operations complete successfully
  - Verify no data corruption in config files
  - Test file locking behavior during long operations

- [ ] **Recovery from corrupted files**:
  - Backup good state: `cp -r ~/.simple-secrets /tmp/good-state`
  - Corrupt a JSON file: echo "invalid json" > ~/.simple-secrets/secrets.json
  - Test error handling: `simple-secrets list keys` (should show clear error)
  - Verify other files not affected by single file corruption
  - Restore: `rm -rf ~/.simple-secrets && mv /tmp/good-state ~/.simple-secrets`

#### Migration & Compatibility

- [ ] **Version compatibility**:
  - Test with existing config files from different versions (if applicable)
  - Verify migration/upgrade behavior works correctly
  - Test that new features don't break existing configs
  - Verify backward compatibility promises are maintained

- [ ] **Config format validation**:
  - Test legacy config format still works
  - Test new config format is properly validated
  - Verify migration from old to new format (if applicable)
  - Test mixed format handling (old + new config files present)

**🚨 CRITICAL SECURITY FAILURES - STOP IMMEDIATELY**:

- Any plaintext secrets or tokens found in config files
- Any encrypted data that doesn't change after key rotation
- Any file permissions that allow other users to read secrets
- Any backup that doesn't restore to identical system state
