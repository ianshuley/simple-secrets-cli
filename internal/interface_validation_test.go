...
// TestBackupRestoreThroughInterface validates backup/restore operations through interfaces
func TestBackupRestoreThroughInterface(t *testing.T) {
	store := newTempStoreForInterface(t)
	...
	originalSecrets := map[string]string{
		"backup/test1": "backup-value1",
		"backup/test2": "backup-value2",
	}

	for key, value := range originalSecrets {
		if err := secretsMgr.Put(key, value); err != nil {
			t.Fatalf("Put failed: %v", err)
		}
	}

	// Create backup through interface
	tmpDir := t.TempDir()
	backupDir := filepath.Join(tmpDir, "manual-backup")
	if err := secretsMgr.CreateBackup(backupDir); err != nil {
		t.Fatalf("CreateBackup failed: %v", err)
	}

	// Modify data after backup
	if err := secretsMgr.Put("backup/test3", "new-value"); err != nil {
		t.Fatalf("Put after backup failed: %v", err)
	}
	if err := secretsMgr.Delete("backup/test1"); err != nil {
		t.Fatalf("Delete after backup failed: %v", err)
	}

	// Restore from backup through interface
	if err := secretsMgr.RestoreFromBackup(backupDir); err != nil {
		t.Fatalf("RestoreFromBackup failed: %v", err)
	}

	// Verify original data is restored through interface
	for key, expectedValue := range originalSecrets {
		value, err := secretsMgr.Get(key)
		if err != nil {
			t.Fatalf("Get after restore failed for key %s: %v", key, err)
		}
		if value != expectedValue {
			t.Errorf("Value mismatch after restore for key %s: got %q, expected %q", key, value, expectedValue)
		}
	}

	// Verify post-backup changes are reverted
	_, err := secretsMgr.Get("backup/test3")
	if err == nil {
		t.Error("Post-backup key should not exist after restore")
	}

	t.Log("Backup/restore through interface completed successfully")
}
