...
// TestConcurrentUserOperations validates that concurrent operations on UserStore
...
func TestConcurrentUserOperations(t *testing.T) {
	...
	permissions := RolePermissions{
		"admin":  {"read", "write", "rotate", "manage-users"},
		"reader": {"read"},
	}
	...
	// Test 1: Concurrent Lookup operations
	t.Run("ConcurrentLookups", func(t *testing.T) {
		...
		wg.Wait()
		t.Log("Concurrent lookup operations completed without deadlocks")
	})

	// Test 2: Concurrent mixed UserStore operations
	t.Run("ConcurrentMixedUserOperations", func(t *testing.T) {
		...
		for range numGoroutines {
			go func() {
				defer wg.Done()
				for range operationsPerGoroutine {
					_, _ = userStore.Lookup("admin-token")
				}
			}()
		}
		...
		// Concurrent Permissions() calls
		for range numGoroutines {
			go func() {
				defer wg.Done()
				for range operationsPerGoroutine {
					_ = userStore.Permissions()
				}
			}()
		}

		wg.Wait()
		t.Log("Concurrent mixed user operations completed without deadlocks")
	})
}
