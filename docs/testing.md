...
- [ ] **Self-rotate token**: `./simple-secrets rotate token` (no username)
...
- [ ] **`./simple-secrets rotate token`** → self token rotation
...
| Self-Rotate Token | `rotate token` | Any User | Own token rotated | | ☐ P ☐ F |
...
| Reader | `rotate` others' tokens | Access denied | | ☐ P ☐ F |
| Reader | `rotate token` (self) | Allowed | | ☐ P ☐ F |
| Admin | `rotate token` (self) | Allowed | | ☐ P ☐ F |
