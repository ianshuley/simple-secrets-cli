...
- **No decorative terminal calls**: Every `echo` command costs money - only use terminal for actual validation
- **No phase interruptions**: Complete current consistency validation fully before responding to user comments
- **Batch operations**: Use multi-replace and parallel operations when possible
...
```bash
env | grep -i simple
# Note any set variables - they affect testing context:
# SIMPLE_SECRETS_TOKEN: Provides auto-authentication
# SIMPLE_SECRETS_CONFIG_DIR: Uses custom config location
```
...
- **Shell autocomplete functionality** (secret name completion, authentication-aware behavior)
- **Token management disambiguation** (token value vs username distinction, disable command variants)
- **Secret generation functionality** (`--generate` flag, `--length` option, character sets, defaults)
...
Ensure consistency for recently added features:

#### **Shell Autocomplete Features**

- [ ] **README.md autocomplete section** matches actual `ValidArgsFunction` behavior in `cmd/*.go`
- [ ] **Completion examples** in documentation work with current secret names
- [ ] **Authentication-aware completion** documented correctly (graceful fallback when unauthenticated)
- [ ] **Testing framework** includes comprehensive autocomplete test cases
- [ ] **Help text** mentions completion capabilities where relevant

#### **Token/User Disambiguation**

- [ ] **disable command help** (`cmd/disable.go`) matches README examples
- [ ] **Token and user subcommand equivalence** documented consistently (both accept usernames and token values)
- [ ] **Token vs username examples** consistent across README and help text for both subcommands
- [ ] **Error messages** for invalid tokens/usernames are helpful and consistent
- [ ] **Testing framework** covers both token value and username disable scenarios for both subcommands
- [ ] **Service layer** (`internal/service.go`, `internal/rbac.go`) properly handles both input types

#### **List Output Formatting**

- [ ] **Quote removal logic** (`safeDisplayFormat` in `cmd/list.go`) matches user expectations
- [ ] **README examples** show clean output without unwanted quotes
- [ ] **Testing examples** reflect actual formatting behavior
- [ ] **Special character handling** documented and tested
